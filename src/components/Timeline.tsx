...
type ToolTab = 'filters' | 'transitions' | 'video' | 'body';
...
const bodyCategories: Category[] = [
  { name: 'Body Outline & Neon', items: [
    { name: 'Electric Rays', id: 'electricRays', desc: 'Tracked electric outline' }, { name: 'Neon Glow Outline', id: 'neonGlowOutline', desc: 'Glowing subject edge' }, { name: 'Lightning Strokes', id: 'lightningStrokes', desc: 'Body-anchored lightning' }, { name: 'Laser Aura', id: 'laserAura', desc: 'Laser body aura' }
  ]},
  { name: 'Face & Head Effects', items: [
    { name: 'Glowing Eyes', id: 'glowingEyes', desc: 'Tracked eye glow' }, { name: 'Halo Ring', id: 'haloRing', desc: 'Head-anchored halo' }, { name: 'Angel Wings', id: 'angelWings', desc: 'Back-anchored wings' }, { name: 'Demon Horns', id: 'demonHorns', desc: 'Head-anchored horns' }, { name: 'Floating Crown', id: 'floatingCrown', desc: 'Head crown tracker' }
  ]},
  { name: 'Clone & Motion Trails', items: [
    { name: 'Disappearing Clone', id: 'disappearingClone', desc: 'Motion clone trail' }, { name: 'Shadow Trail', id: 'shadowTrail', desc: 'Tracked shadow' }, { name: 'RGB Echo', id: 'rgbEcho', desc: 'Chromatic motion echo' }, { name: 'Speed Phantom', id: 'speedPhantom', desc: 'Velocity ghosting' }
  ]},
  { name: 'Energy & Aura', items: [
    { name: 'Super Saiyan Flame', id: 'superSaiyanFlame', desc: 'Energy flame aura' }, { name: 'Particle Burst', id: 'particleBurst', desc: 'Tracked particles' }, { name: 'Galaxy Sparkles', id: 'galaxySparkles', desc: 'Cosmic particles' }, { name: 'Cosmic Pulse', id: 'cosmicPulse', desc: 'Pulsing aura' }
  ]},
  { name: 'Background Isolation', items: [
    { name: 'Background Matrix', id: 'backgroundMatrix', desc: 'Subject-isolated matrix' }, { name: 'Fire Ring Behind', id: 'fireRingBehind', desc: 'Behind-subject ring' }, { name: 'Grid Aura', id: 'gridAura', desc: 'Tracked grid aura' }
  ]}
];
...
export const Timeline: React.FC<TimelineProps> = (props) => {
  ...
  const [trim, setTrim] = useState<{ id: string; side: 'start' | 'end' } | null>(null); const [toolTab, setToolTab] = useState<ToolTab>('filters'); const [category, setCategory] = useState('All'); const [editingTextId, setEditingTextId] = useState<string | null>(null); const [effectsOpenId, setEffectsOpenId] = useState<string | null>(null);
  const bodyRef = useRef<HTMLDivElement>(null); const timelineWidth = Math.max(900, duration * zoom + 220); const selectedClip = clips.find(c => c.id === selectedClipId) || null; const showEffects = !!selectedClip && effectsOpenId === selectedClip.id && (selectedClip.type === 'video' || selectedClip.type === 'image');
  ...
  const categories = toolTab === 'filters' ? filterCategories : toolTab === 'transitions' ? transitionCategories : toolTab === 'video' ? videoCategories : bodyCategories;
